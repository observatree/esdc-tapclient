...
CREATE OR REPLACE FUNCTION tap_schema.crossmatch_positional(
    table_schema_a character varying,
    table_name_a character varying,
    table_schema_b character varying,
    table_name_b character varying,
    radius double precision,
    output_table_schema character varying,
    output_table_name character varying)
  RETURNS bigint AS
$BODY$
...
-- Create INDEXES (NOT Foreign Keys) for foreign columns
-- If foreign keys are created, table gets locked when selects are performed on referenced tables, so no deletion is possible.

CREATE INDEX ON '||output_table_schema||'.'||output_table_name||' ('||small_table_name || '_' || small_table_oid || ');

CREATE INDEX ON '||output_table_schema||'.'||output_table_name||' ('||large_table_name|| '_' || large_table_oid || ');

-- Create estimator index
CREATE INDEX ON '||output_table_schema||'.'||output_table_name||' (dist);

-- Insert table in TAP_SCHEMA
INSERT INTO tap_schema.all_tables (table_name, schema_name, table_type, description, utype, size, flags) VALUES 
	('||quote_literal(output_table_name)||', '||quote_literal(output_table_schema)||', ''table'', 
...
EXECUTE 'SELECT uws2_schema.db_table_usage($1,$2)'
INTO output_size
USING output_table_schema, output_table_name;
